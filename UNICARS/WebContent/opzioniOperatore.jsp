<img src="image/label/gestioneVeicoli.png">
<ul>
	<li><a class="silver" href="index.jsp?id=inserisciVeicolo">Inserisci</a></li>
	<li><a class="silver" href="index.jsp?id=modificaVeicolo">Modifica</a></li>
	<li><a class="silver" href="index.jsp?id=eliminaVeicolo">Elimina</a></li>
	<li><a class="silver" href="index.jsp?id=visualizzaParcoVeicoli">Visualizza Parco</a></li>
</ul>
<img src="image/label/gestioneRiparazioni.png">
<ul>
	<li><a class="silver" href="index.jsp?id=inserisciRiparazione">Inserisci</a></li>
	<li><a class="silver" href="index.jsp?id=modificaRiparazione">Modifica</a></li>
	<li><a class="silver" href="index.jsp?id=eliminaRiparazione">Elimina</a></li>
	<li><a class="silver" href="index.jsp?id=visualizzaStatoRiparazioni">Visualizza Stato</a></li>
</ul>
<img src="image/label/gestioneClienti.png">
<ul>
	<li><a class="silver" href="index.jsp?id=inserisciCliente">Inserisci</a></li>
	<li><a class="silver" href="index.jsp?id=modificaCliente">Modifica</a></li>
	<li><a class="silver" href="index.jsp?id=eliminaCliente">Elimina</a></li>
</ul>
<img src="image/label/gestioneVendite.png">
<ul>
	<li><a class="silver" href="index.jsp?id=inserisciVendita">Inserisci</a></li>
	<li><a class="silver" href="VisualizzaVendite?azione=modifica">Modifica</a></li>
	<li><a class="silver" href="index.jsp?id=eliminaVendita1">Elimina</a></li>
</ul>
<img src="image/label/gestioneAppuntamenti.png">
<ul>
	<li><a class="silver" href="index.jsp?id=inserisciAppuntamento">Inserisci</a></li>
	<li><a class="silver" href="index.jsp?id=modificaAppuntamento1">Modifica</a></li>
	<li><a class="silver" href="index.jsp?id=eliminaAppuntamento1">Elimina</a></li>
</ul>